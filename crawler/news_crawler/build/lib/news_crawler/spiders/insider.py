...
class InsiderSpider(SitemapSpider):
    name = 'insider'
    allowed_domains = ['insider.com']
    sitemap_urls = ['https://www.insider.com/sitemap/google-news.xml']

    def parse(self, response):
        item = NewsCrawlerItem()
        item['url'] = response.url
        item['title'] = response.xpath('//title/text()').get()
        item['date'] = parse(response.css('meta[name="datePublished"]::attr(content)').get()).isoformat()

        text_nodes = response.css('section.post-content p ::text').getall()
