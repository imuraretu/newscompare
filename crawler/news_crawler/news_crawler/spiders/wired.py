...
class WiredSpider(SitemapSpider):
    ...
    sitemap_urls = ['https://www.wired.co.uk/feed/google-latest-news/sitemap-google-news']
    custom_settings = {
    ...
    def _parse_sitemap(self, response):
        ...
        urls = response.xpath('//ns:url[news:news/news:publication/news:language/text()="en-GB"]/ns:loc/text()', namespaces=ns).extract()
        print("====================")
        print(urls)
        for url in urls:
            if url.endswith('.xml'):
