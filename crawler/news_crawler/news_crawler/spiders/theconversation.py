...
class TheConversationSpider(SitemapSpider):
    name = 'theconversation'
    allowed_domains = ['theconversation.com']
    sitemap_urls = ['https://theconversation.com/us/sitemap_news.xml']
    custom_settings = {
    ...
    def parse(self, response):
        ...
        item['date'] = parse(response.css('meta[property="og:updated_time"]::attr(content)').get()).isoformat()

        text_nodes = response.css('article div.wrapper p ::text').getall()
