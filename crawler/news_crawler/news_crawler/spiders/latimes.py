...
class LATimesSpider(SitemapSpider):
    name = 'latimes'
    allowed_domains = ['latimes.com']
    sitemap_urls = ['https://latimes.com/news-sitemap-content.xml']
    ...
    def parse(self, response):
        ...
        text_nodes = response.css('div.story-body p ::text').getall()
    ...
    def _parse_sitemap(self, response):
        ...
        urls = response.xpath('//ns:url[news:news/news:publication/news:language/text()="eng"]/ns:loc/text()', namespaces=ns).extract()
        print(urls)
        for url in urls:
            if url.endswith('.xml'):
